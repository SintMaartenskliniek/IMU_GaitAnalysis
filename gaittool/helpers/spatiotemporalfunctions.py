...
Last update: May 2023 (Jean Ormiston)
...
def swingtime(data, errors):
    ...
    # ADJUSTED: Jean Ormiston, May 2023 for steps outside turns
    ...
    if errors['No walking period'] == False:
        for i in range(0, len(sides)):
            if sides[i] == 'left':
                ...
                data['Left foot']['Gait Phases']['Swing time (samples)'] = np.zeros((len(indexHeelStrike) - 1, 3))
                for hs in range(0, len(indexHeelStrike) - 1):
                    data['Left foot']['Gait Phases']['Swing time (samples)'][hs, 0] = indexToeOff[hs]
                    data['Left foot']['Gait Phases']['Swing time (samples)'][hs, 1] = indexHeelStrike[hs]
                data['Left foot']['Gait Phases']['Swing samples'], data['Left foot']['Gait Phases'][
                                                                       'Swing time (samples)'][:, 2], \
                data['Spatiotemporals']['Swing time left (s)'] = functionswingtime(indexToeOff, indexHeelStrike,
                                                                                   sample_frequency)

            elif sides[i] == 'right':
                indexToeOff = data['Right foot']['Gait Events']['Terminal Contact']
                indexHeelStrike = data['Right foot']['Gait Events']['Initial Contact']
                data['Right foot']['Gait Phases']['Swing time (samples)'] = np.zeros((len(indexHeelStrike) - 1, 3))
                for hs in range(0, len(indexHeelStrike) - 1):
                    data['Right foot']['Gait Phases']['Swing time (samples)'][hs, 0] = indexToeOff[hs]
                    data['Right foot']['Gait Phases']['Swing time (samples)'][hs, 1] = indexHeelStrike[hs]
                data['Right foot']['Gait Phases']['Swing samples'], data['Right foot']['Gait Phases'][
                                                                        'Swing time (samples)'][:, 2], \
                data['Spatiotemporals']['Swing time right (s)'] = functionswingtime(indexToeOff, indexHeelStrike,
                                                                                    sample_frequency)
    else:
...
def stridetime(data, errors):
    ...
    if errors['No walking period'] == False:
        for i in range(0, len(sides)):
            if sides[i] == 'left':
                indexHeelStrike = data['Left foot']['Gait Events']['Initial Contact']
                data['Left foot']['Gait Phases']['Stride time per stride (samples)'] = np.zeros(
                    (len(indexHeelStrike) - 1, 3))
                for hs in range(0, len(indexHeelStrike) - 1):
                    data['Left foot']['Gait Phases']['Stride time per stride (samples)'][hs, 0] = indexHeelStrike[hs]
                    data['Left foot']['Gait Phases']['Stride time per stride (samples)'][hs, 1] = indexHeelStrike[
                        hs + 1]
                data['Spatiotemporals']['Stride time left (s)'], data['Left foot']['Gait Phases'][
                                                                     'Stride time per stride (samples)'][:,
                                                                 2] = functionstridetime(indexHeelStrike,
                                                                                         sample_frequency)

            elif sides[i] == 'right':
                indexHeelStrike = data['Right foot']['Gait Events']['Initial Contact']
                data['Right foot']['Gait Phases']['Stride time per stride (samples)'] = np.zeros(
                    (len(indexHeelStrike) - 1, 3))
                for hs in range(0, len(indexHeelStrike) - 1):
                    data['Right foot']['Gait Phases']['Stride time per stride (samples)'][hs, 0] = indexHeelStrike[hs]
                    data['Right foot']['Gait Phases']['Stride time per stride (samples)'][hs, 1] = indexHeelStrike[
                        hs + 1]
                data['Spatiotemporals']['Stride time right (s)'], data['Right foot']['Gait Phases'][
                                                                      'Stride time per stride (samples)'][:,
                                                                  2] = functionstridetime(indexHeelStrike,
                                                                                          sample_frequency)
    ...
    if errors['No walking period'] == False:
        ...
        for i in range(0,len(indexHeelStrikeRight)):
            ...
            if len(lastHSleft) > 0:
                ...
                data['Right foot']['Gait Phases']['Step time (samples)'][i, 0] = indexHeelStrikeLeft[lastHSleft]
                data['Right foot']['Gait Phases']['Step time (samples)'][i, 1] = indexHeelStrikeRight[i]
                ...
                data['Right foot']['Gait Phases']['Step time (samples)'][i, 2] = indexHeelStrikeRight[i] - indexHeelStrikeLeft[lastHSleft]
        StepTime = np.nanmedian(data['Right foot']['Gait Phases']['Step time (samples)'][:, 2])/sample_frequency
        ...
        for i in range(0,len(indexHeelStrikeLeft)):
            ...
            if len(lastHSright) > 0:
                ...
                data['Left foot']['Gait Phases']['Step time (samples)'][i, 0] = \
                indexHeelStrikeRight[lastHSright]
                data['Left foot']['Gait Phases']['Step time (samples)'][i, 1] = \
                indexHeelStrikeLeft[i]
                ...
                data['Left foot']['Gait Phases']['Step time (samples)'][i, 2] = indexHeelStrikeLeft[i] - indexHeelStrikeRight[lastHSright]
        StepTime = st.median(data['Left foot']['Gait Phases']['Step time (samples)'][:, 2])/sample_frequency
...
def turnidentification(data):
    ...
    if np.max(np.abs(fGYRz)) > 70/180*np.pi:
        threshold = 0.3*np.max(np.abs(fGYRz)) #Normally = 0.3*
...
def stridelength(data, errors, showfigure):
    ...
    #TODO: Why removal of steps below when stridesinturn full of nan?? And no turns detected
    ...
    data['Right foot']['Gait Phases']['Step time - no 2 steps around turn (samples)'] = np.transpose(np.array([[], [], []]))
    data['Left foot']['Gait Phases']['Step time - no 2 steps around turn (samples)'] = np.transpose(np.array([[], [], []]))
    data['Right foot']['Gait Phases']['Stride time - no 2 steps around turn (samples)'] = np.transpose(
        np.array([[], [], []]))
    data['Left foot']['Gait Phases']['Stride time - no 2 steps around turn (samples)'] = np.transpose(
        np.array([[], [], []]))
    data['Right foot']['Gait Phases']['Swing time - no 2 steps around turn (samples)'] = np.transpose(
        np.array([[], [], []]))
    data['Left foot']['Gait Phases']['Swing time - no 2 steps around turn (samples)'] = np.transpose(
        np.array([[], [], []]))
    ...
    data['Spatiotemporals']['Gait events 2 steps before and after turn'] = remove
    for j in range(2, len(data['Left foot']['derived']['Stride length - straight walking (m)'])-2):
        #Added by Jean Ormiston 01/05/2023 for step time
        if data['Left foot']['Gait Phases']['Step time (samples)'][j,1] not in remove:
            data['Left foot']['Gait Phases']['Step time - no 2 steps around turn (samples)'] = np.vstack((data['Left foot']['Gait Phases']['Step time (samples)'], data['Left foot']['Gait Phases']['Step time (samples)'][j,:]))
        # Added by Jean Ormiston 01/05/2023 for stride time
        if data['Left foot']['Gait Phases']['Stride time per stride (samples)'][j, 1] not in remove:
            data['Left foot']['Gait Phases']['Stride time - no 2 steps around turn (samples)'] = np.vstack((data[
                                                                                                              'Left foot'][
                                                                                                              'Gait Phases'][
                                                                                                              'Stride time per stride (samples)'],
                                                                                                          data[
                                                                                                              'Left foot'][
                                                                                                              'Gait Phases'][
                                                                                                              'Stride time per stride (samples)'][j, :]))
        # Added by Jean Ormiston 01/05/2023 for swing time
        if data['Left foot']['Gait Phases']['Swing time (samples)'][j, 1] not in remove:
            data['Left foot']['Gait Phases']['Swing time - no 2 steps around turn (samples)'] = np.vstack((data[
                                                                                                                'Left foot'][
                                                                                                                'Gait Phases'][
                                                                                                                'Swing time (samples)'],
                                                                                                            data[
    ...
    for j in range(2, len(data['Right foot']['derived']['Stride length - straight walking (m)'])-2):
        # Added by Jean Ormiston 01/05/2023 for step time
        if data['Right foot']['Gait Phases']['Step time (samples)'][j, 1] not in remove:
            data['Right foot']['Gait Phases']['Step time - no 2 steps around turn (samples)'] = np.vstack((data[
                                                                                                                  'Right foot'][
                                                                                                                  'Gait Phases'][
                                                                                                                  'Step time (samples)'],
                                                                                                              data[
                                                                                                              ...
                                                                                                              j, :]))
        # Added by Jean Ormiston 01/05/2023 for stride time
        if data['Right foot']['Gait Phases']['Stride time per stride (samples)'][j, 1] not in remove:
            data['Right foot']['Gait Phases']['Stride time - no 2 steps around turn (samples)'] = np.vstack((data[
                                                                                                                'Right foot'][
                                                                                                                'Gait Phases'][
                                                                                                                'Stride time per stride (samples)'],
                                                                                                            data[
                                                                                                            ...
                                                                                                            j, :]))
        # Added by Jean Ormiston 01/05/2023 for swing time
        if data['Right foot']['Gait Phases']['Swing time (samples)'][j, 1] not in remove:
            data['Right foot']['Gait Phases']['Swing time - no 2 steps around turn (samples)'] = np.vstack((data[
                                                                                                                 'Right foot'][
                                                                                                                 'Gait Phases'][
                                                                                                                 'Swing time (samples)'],
                                                                                                             data[
    ...
    #Copied from step time function Jean Ormiston 01/05/2023
    #Check if
    if np.isnan(stridesinturn):
        StepTime = st.median(data['Left foot']['Gait Phases']['Step time - no 2 steps around turn (samples)'][:,2]) / sample_frequency
        data['Spatiotemporals']['Step time left - no 2 steps around turn (s)'] = round(StepTime, 2)
        StepTime = st.median(data['Right foot']['Gait Phases']['Step time - no 2 steps around turn (samples)'][:,2]) / sample_frequency
        data['Spatiotemporals']['Step time right - no 2 steps around turn (s)'] = round(StepTime, 2)

        SwingTime = st.median(
            data['Left foot']['Gait Phases']['Swing time - no 2 steps around turn (samples)'][:, 2]) / sample_frequency
        data['Spatiotemporals']['Swing time left - no 2 steps around turn (s)'] = round(SwingTime, 2)
        SwingTime = st.median(
            data['Right foot']['Gait Phases']['Swing time - no 2 steps around turn (samples)'][:, 2]) / sample_frequency
        data['Spatiotemporals']['Swing time right - no 2 steps around turn (s)'] = round(SwingTime, 2)

        StrideTime = st.median(
            data['Left foot']['Gait Phases']['Stride time - no 2 steps around turn (samples)'][:, 2]) / sample_frequency
        data['Spatiotemporals']['Stride time left - no 2 steps around turn (s)'] = round(StrideTime, 2)
        StrideTime = st.median(
            data['Right foot']['Gait Phases']['Stride time - no 2 steps around turn (samples)'][:, 2]) / sample_frequency
        data['Spatiotemporals']['Stride time right - no 2 steps around turn (s)'] = round(StrideTime, 2)
    ...
    if len(ends) > 0:
        ...
        for i in range(0, np.shape(transfers)[0]):
            ...
            if np.nanstd(lumbarvelocity[initiation,2]) < np.nanstd(lumbarvelocity[termination,2]):
                if np.nanstd(lumbaracc[initiation,2]) < 0.5:
            ...
            # Assume vertical velocity variation of lumbar sensor larger at the start of an stand-to-sit transfer compared to the end and smaller than 0.5 m/s
                ...
                if np.nanstd(lumbaracc[termination,2]) < 0.5:
